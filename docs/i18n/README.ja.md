...
```bash
# 先にリリースファイルから ochenstarik-server-monitor-manager.sh をダウンロードしてください。
...
`v0.1.0-alpha.14` はテスト版です。現在の開発ブランチには Windows client、Hub/Node installer、Links、mTLS、証明書失効と再登録、SQLite、監査、イベント、downsampling 付きの制限オフラインバッファが実装済みです。再接続調整、50–100 Node 負荷試験、署名付き Windows installer は今後の課題です。
